...
public class JsonUtil {
	...
	*/
	private static func toJsonByObj(_ object: Any) -> Any {
		...
		if let superMirrorData = superMirror {
            for (name, value) in superMirrorData.children {
                if let name = name {
                    dict[name] = value
                }
			}
	...
	*/
	private static func vHandler(_ v: Any) -> Any {
		...
		} else if let vData = v as? Data {
            return "\"\(stringReplace(source: String(data: vData, encoding: .utf8) ?? ""))\""
        } else if let vData = v as? [String: Any] {
			var result = "{"
			// 解析键值对
